...
| 2 | `PlainsightAI/jester-pipelines` | `deploy/**/base/deployment.yaml`, `deploy/**/overlays/*/patch.yaml` | `image: us-west1-docker.pkg.dev/plainsightai-prod/oci/<filter>:<tag>` (GAR) | YAML in-place rewrite of `<tag>` | **No** — dropped (see Section 2): effectively dormant (last real push 2026-01), and its GAR pins depend on an unconfirmed mirror sync (Open question 1) |
